...
|---|---|---|
| The local model is not fine-tuned | `qwen2.5:3b` is pulled and auto-detected, and the assistant, the Copilot's prose and Tier-3 rephrasing all run against it. It knows the project by retrieval over the project's own documents at question time, with sources attached and a figure check, not by training | Fine-tuning a 3B model needs a labelled corpus of questions and grounded answers that does not exist yet, and would bake in a snapshot of documents that change every milestone. Retrieval keeps the model honest and current; a LoRA on top of it is the next step once there is a corpus |
