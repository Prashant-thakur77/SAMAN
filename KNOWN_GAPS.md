...
| Smart-Create does not show a substitute's approval | The item page and the Substitutes screen label every equivalence as approved, rejected or awaiting engineering approval. Smart-Create's "interchangeable parts" list does not: a description being typed is a probe, not an item, so no relation row exists to carry the decision | Would need the check to look up approvals on the suggested item's existing relations; a small join, not yet done |
...
| No bin labels | The CNMC has a check digit made for scanning; nothing prints it | A label sheet with a Code 39 or QR rendering of the code is a small SVG, not yet drawn |
