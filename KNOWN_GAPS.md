...
|---|---|---|
| The language model is not fine-tuned; the pairwise model is trained | `qwen2.5:3b` is retrieval-grounded, not trained. The model that *is* trained is `learn.py`: a fifteen-weight logistic regression on Workbench decisions, saved as readable JSON, ordering the grey queue and scored on the held-out split (0.997 AUC vs the pipeline's 0.702 on the demo profile). Its labelled pairs export as the corpus a LoRA would need | A LoRA on the language model needs that corpus to exist first, and today it holds 400 *simulated* labels plus whatever reviewers have decided. Fine-tuning on simulated answers would teach the model the seed generator. When real decisions accumulate, the corpus is one download away |
