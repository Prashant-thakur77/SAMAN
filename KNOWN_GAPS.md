...
| Voice input uses the browser's engine | The assistant's microphone calls `SpeechRecognition`, which in Chrome is a Google service; Firefox has none. Speech output is local on every platform | A local speech model would keep the no-network guarantee in every browser; for a prototype the browser's engine is honest as long as it is named, so the control appears only where the API exists and the README says where the audio goes |
