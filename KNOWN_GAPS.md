...
|---|---|---|
| The language model is not fine-tuned; the pairwise model is trained | `qwen2.5:3b` is retrieval-grounded, not trained. The model that *is* trained is `learn.py`: a twenty-four-weight logistic regression on Workbench decisions, saved as readable JSON, ordering the grey queue, retrained as a champion/challenger loop, and scored on the held-out split (0.999 AUC vs the pipeline's 0.702 on the demo profile). Its labelled pairs export as the corpus a LoRA would need | A LoRA on the language model needs that corpus to exist first, and today it holds 400 *simulated* labels plus whatever reviewers have decided. Fine-tuning on simulated answers would teach the model the seed generator. When real decisions accumulate, the corpus is one download away |
| Threshold suggestions are computed, never applied, and are only as good as the labels | `learn.suggest_thresholds` reports per class the confidence cut that maximises F1 on the labelled pairs beside the current `T_HIGH`; the admin page marks them *suggested, not applied* | The labelled pairs are the ones reviewers (or the simulator) happened to answer, not a sample of the estate, so a cut fitted to them can be badly off for a class with few labels; and F1 is not the objective an auto-merge threshold serves. Applying them by hand, with the audit log, is the design, not a shortcut left for later |
| The demo's reviewer labels are simulated | `make demo` and **Simulate reviewers** label tuning-split pairs from the seed's ground truth, written as `source=simulated`, never the held-out split and never a task | The learned model needs labels to exist before anyone has reviewed; the source column keeps the two kinds apart, and the README's numbers say which they used |
| No live SAP system has been connected | `ErpAdapter` is the contract; `MockErpAdapter` serves the demo; `RfcErpAdapter` implements the contract over `pyrfc` (RFC_READ_TABLE, BAPI_MATERIAL_SAVEDATA, BAPI_TRANSACTION_COMMIT) and is tested against a fake connection that records every call; the dry run exports LSMW/LTMC load files; an API key lets a BAdI call Smart-Create at MM01 (docs/sap-integration.md) | The SAP NetWeaver RFC SDK is proprietary and no SAP system was available to this build. The adapter follows the connector's documented call shapes; the first run against a real system is a basis team's afternoon, not a redesign |
...
| ~~Smart-Create does not show a substitute's approval~~ **closed** | Each interchangeable part now carries the decision on the equivalence between it and the existing record the check found (`smart_create._approvals`): approved with the engineer's reason, proposed, rejected, or none on record | A probe still has no relation of its own; the join reads the suggested record's, which is the decision that applies to fitting the part |
...
| Camera input is measured on rendered plates (the same holds for the browser reader on the Scan screen) | 0.967 on a clean plate and 0.300 on a phone-quality one, held out. But the plates are drawn and then degraded, not photographed. A scratched bearing race under warehouse light is untested | It would need images nobody has. The report says which question it answers rather than implying the other one |
...
| Voice needs `make deps-stt` and `make deps-tts` for the local engines | With them, speech is recognised by faster-whisper and spoken by piper on the server, verified end to end in headless Chromium with a fake microphone and an instrumented audio element. Without them the widget falls back to the browser's engines, and on Linux Chromium the browser has no voices at all, so the speaker button says so rather than speaking to nobody | About 200 MB of weights and a voice are not committed; the fallback and its limits are named in the UI and here so nobody mistakes them for the local path |
| Realised savings are not tracked | The opportunity dashboard identifies savings (joint tenders, transfers, dead stock) under a stated capture assumption. Nothing records what was actually paid after a consolidation | Needs post-consolidation purchase orders flowing back from the ERP; the RFC adapter reads EKPO, so the data path exists, but no ledger compares realised prices to the baseline yet |
| ~~Vendors are not de-duplicated~~ **closed** | `app/vendors.py` reduces a name to its company (courtesy prefixes, legal suffixes and house spellings removed, a hand-kept alias table for groups) before the overlap analysis groups on it; the spelling shown is the commonest, the others are listed beside it | Kept for the record: the synthetic estate's 25 vendors were already clean, so the folded count on the Vendors tab reads 0 there; a real extract is where it earns its place |
| No signed bundles between CPSE nodes | Restricted mode exchanges Bloom fingerprints; approved golden records travel as CSV exports | A federation of on-premises nodes and one registrar needs signed, versioned bundles of golden records and codes; the export exists, the signature and the import do not |
| ~~No bin labels~~ **closed** | `/labels/<code>` prints a QR and a Code 128 of the CNMC with the legacy codes beside it, and `/scan` reads it back on a phone, a laptop camera or a barcode gun (`app/scan.py`) | Kept for the record: the check digit was designed for exactly this, and the round trip from label to lookup is now tested end to end in a real browser |
