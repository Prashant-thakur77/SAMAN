...
| Voice input needs `make deps-stt` for the local engine | With it, speech is transcribed on the server by faster-whisper and never leaves the machine, verified end to end with a fake microphone in headless Chromium. Without it the widget falls back to the browser's `SpeechRecognition`, which in Chrome is a Google service and in Firefox does not exist | The 140 MB of weights are not committed; the fallback is named in the UI and the README so nobody mistakes it for the local path. Speech output is the browser's `speechSynthesis` on every platform |
