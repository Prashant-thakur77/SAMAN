<h1 align="center">SAMAN</h1>

<p align="center"><strong>Standardised Asset &amp; Material Analysis Network</strong><br/><em>One Nation, One Material Code</em></p>
...
company: `BALL BEARING SKF 6205-2Z` in one material master, `BRG,BALL,6205ZZ,SKF`
in another, `बेयरिंग 6205 ZZ SKF` in a third. SAMAN ingests those catalogues,
finds duplicates and functional equivalents across them, drafts one golden
record per real item, routes the uncertain matches through human review, and
issues a **Common National Material Code (CNMC)** for each. It then keeps the
duplicates from coming back: every new material request is checked against
the whole registry before a code is raised.

SAMAN is the platform; the CNMC is the code it issues. It runs **offline, on a
laptop or a CPSE's own server**, with no cloud calls and no API keys; every
optional engine degrades to a working fallback and says so on screen. Nothing
decides unaccountably: a rules-based veto layer overrides any score, a person
decides every grey-band pair, and every mutation is on a hash-chained ledger.

| Measured on the held-out 40% of a 12,000-row, five-CPSE synthetic estate | Result | Target |
|---|---|---|
| Duplicate precision / recall (pairwise) | **0.997 / 0.960** | ≥ 0.92 / ≥ 0.80 |
| B-cubed F1 (cluster level) | **0.991** | — |
| Blocking recall | **0.994** | ≥ 0.97 |
| Veto precision on planted traps | **1.000** | ≥ 0.98 |
| Decided without a human | **99.4 %** of scored pairs | — |
| Naive exact-text baseline, same data | P 1.000 · R 0.029 | — |

*Thresholds were tuned on the other 60 % and frozen; nothing above was tuned
against these numbers. All data is synthetic, with full ground truth, and
...
```mermaid
flowchart LR
    X[CSV / Excel<br/>SAP extracts] --> N[normalise<br/>+ extract] --> B[block<br/>7 passes] --> M[match<br/>4 tiers + veto]
    M -->|high| C[cluster →<br/>golden record]
    M -->|grey| W[review queue<br/>a person decides] --> C
    M -->|low| D[distinct]
    C --> K[CNMC issued] --> S[SAP cross-reference<br/>reversible]
    K --> P[Smart-Create<br/>checks every new request]
    V[visibility · audit chain · memo] -.-> C
```

React 18 + TypeScript in front, FastAPI + SQLAlchemy behind, SQLite on disk,
26 API routers over pure engine modules that are tested without the web
layer. The full picture, in six diagrams (context, components, pipeline,
scoped request, deployment shapes, ledger), is
...
| [docs/sap-integration.md](docs/sap-integration.md) | the three SAP doors: RFC adapter, BAdI hook, load files |
| [KNOWN_GAPS.md](KNOWN_GAPS.md) | every gap and every finding, kept public |
| [CONTRIBUTING.md](CONTRIBUTING.md) | how to run the checks and how changes are made |
| `/api/docs` on a running instance | the API reference |

The rest of this file is the long read: how each part works, what it
measures, and why it was built the way it was.

<details>
<summary><strong>Contents</strong></summary>

- [Run it](#run-it)
- [Deploying to a link](#deploying-to-a-link)
...
quality check. `make check` runs everything CI does: ruff, 1,320 backend tests,
141 frontend tests, the type-check and the licence gate. `make demo-snapshot`
