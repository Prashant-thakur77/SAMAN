...
class TestClusterDetail:
    def test_unknown_cluster_is_404(self, as_viewer, pipeline_run):
        assert as_viewer.get("/api/clusters/999999").status_code == 404

    def test_the_golden_record_is_returned_with_its_template(self, as_viewer, multi_member_cluster):
        body = as_viewer.get(f"/api/clusters/{multi_member_cluster}").json()
    ...
    def test_the_golden_description_follows_the_template_not_a_member(
        self, as_viewer, multi_member_cluster
    ):
    ...
    def test_every_fused_field_carries_provenance(self, as_viewer, multi_member_cluster):
    ...
    def test_provenance_shows_the_competing_candidates(self, as_viewer, multi_member_cluster):
        body = as_viewer.get(f"/api/clusters/{multi_member_cluster}").json()
    ...
    def test_a_standardization_delta_is_returned_per_member(self, as_viewer, multi_member_cluster):
