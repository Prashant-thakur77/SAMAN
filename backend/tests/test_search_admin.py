...
class TestSearch:
    def test_a_designation_finds_its_variants_across_cpses(self, as_viewer, db, pipeline_run):
        ...
        body = as_viewer.get(f"/api/items?search={designation}&limit=50").json()
        assert body["total"] >= 1
        assert all(
    ...
    def test_a_barcode_substring_is_not_a_match(self, as_viewer, db, pipeline_run):
        """Tokens must start where the search term does."""
        from app.models import Item

        gtin = db.execute(select(Item.gtin).where(Item.gtin.is_not(None)).limit(1)).scalar()
        middle = gtin[3:7]
        for hit in as_viewer.get(f"/api/items?search={middle}&limit=20").json()["items"]:
    ...
    def test_every_word_must_appear(self, as_viewer, pipeline_run):
        one = as_viewer.get("/api/items?search=6205").json()["total"]
        two = as_viewer.get("/api/items?search=6205%20SKF").json()["total"]
        assert two <= one, "adding a word must narrow the result, not widen it"

    def test_filters_apply(self, as_viewer, pipeline_run):
        by_cpse = as_viewer.get("/api/items?cpse=CPCL&limit=10").json()
        assert all(hit["cpse"] == "CPCL" for hit in by_cpse["items"])
        by_class = as_viewer.get("/api/items?class=valve.gate&limit=10").json()
        assert all(hit["class_code"] == "valve.gate" for hit in by_class["items"])

    def test_pagination_is_server_side_with_a_total(self, as_viewer, pipeline_run):
        """§8A: a 150k-row estate cannot be filtered in the browser."""
        first = as_viewer.get("/api/items?limit=5&offset=0").json()
        second = as_viewer.get("/api/items?limit=5&offset=5").json()
        assert first["total"] == second["total"] > 5
        assert {i["item_id"] for i in first["items"]}.isdisjoint(
            {i["item_id"] for i in second["items"]}
        )

    def test_the_page_size_is_capped(self, as_viewer, pipeline_run):
        assert as_viewer.get("/api/items?limit=5000").status_code == 422

    def test_facets_come_from_the_data(self, as_viewer, pipeline_run):
        facets = as_viewer.get("/api/facets").json()
        assert facets["cpses"] and facets["classes"]
        assert facets["totals"]["items"] > 0

    def test_an_empty_search_returns_the_catalogue(self, as_viewer, pipeline_run):
        assert as_viewer.get("/api/items?limit=3").json()["total"] > 0
...
class TestOnboarding:
    ...
    def test_registering_the_same_code_twice_is_refused(self, as_registrar, pipeline_run):
        ...
        assert (
            as_registrar.post("/api/cpses", json={"code": "DUPCO", "name": "Dup"}).status_code
            == 409
        )
    ...
    def test_a_new_cpses_catalogue_reaches_the_review_queue(self, as_registrar, db, pipeline_run):
...
class TestAdmin:
    ...
    def test_a_role_can_be_changed(self, as_registrar, db, pipeline_run, monkeypatch):
        # The seeded accounts are fixed while the shared demo login is on
        # (see admin.update_user); this is the real-deployment path.
        monkeypatch.setattr(get_settings(), "saman_demo_login", False)
    ...
    def test_you_cannot_disable_your_own_account(self, as_registrar, db, pipeline_run):
        me = db.execute(select(User).where(User.email == "registrar@min.gov.in")).scalar_one()
        response = as_registrar.patch(f"/api/users/{me.id}", json={"active": False})
        assert response.status_code == 409

    def test_disabling_a_user_stops_them_signing_in(
        self, as_registrar, client, db, pipeline_run, monkeypatch
    ):
        # The seeded accounts are fixed while the shared demo login is on
        # (see admin.update_user); this is the real-deployment path.
        monkeypatch.setattr(get_settings(), "saman_demo_login", False)
...
class TestSovereignMode:
    ...
    def test_a_steward_cannot_toggle_it(self, as_steward, pipeline_run):
        assert as_steward.post("/api/settings/sovereign", json={"enabled": True}).status_code == 403
